...
async def on_chat_start():
    ...
    await cl.Message(
        content=(
            "Hi! Send text, image, or voice notes.\n"
            "Tip: ask explicitly with 'send a voice note' or 'generate an image of ...' for media responses."
        )
    ).send()
...
async def on_message(message: cl.Message):
    ...
    if output_state.values.get("workflow") == "audio":
        await cl.Message(content="Generating your voice note...").send()
        ...
        await cl.Message(content="Generating your image...").send()
        response = output_state.values["messages"][-1].content
        image_buffer = output_state.values.get("image_buffer")
        if image_buffer is not None:
            image = cl.Image(content=image_buffer, mime="image/png", display="inline")
        else:
